...
(function() {
     ...
     */
    function handleBulbClick(lightId, lightData, event) {
        ...
        clickTimer = setTimeout(() => {
            ...
            if (data) {
                show(data.lightId, data.lightData, event.target);
            }
        }, CLICK_DELAY);
    }
     ...
     */
    function show(lightId, lightData, bulbElement) {
     ...
     */
    function createPopup(x, y, lightData, hasColor) {
        ...
        bg.setAttribute('x', 0);
        bg.setAttribute('y', 0);
        bg.setAttribute('width', POPUP_WIDTH);
        bg.setAttribute('height', POPUP_HEIGHT);
        ...
        if (hasColor) {
            const wheelGroup = createColorWheel(POPUP_WIDTH / 2, yOffset + COLOR_WHEEL_RADIUS, COLOR_WHEEL_RADIUS, lightData);
        ...
        whiteLabel.setAttribute('y', yOffset);
        ...
        if (hasColor) {
            ...
            colorLabel.setAttribute('y', yOffset);
        ...
        briLabel.setAttribute('y', yOffset);
     ...
     * Create close button
     */
    function createCloseButton(x, y) {
     ...
     */
    function createColorWheel(cx, cy, radius, lightData) {
        const group = document.createElementNS(NS, 'g');
     ...
     */
    function createSwatchRow(presets, x, y, isWhite) {
        const group = document.createElementNS(NS, 'g');
        const swatchSize = 24;
        const gap = 8;
        ...
        presets.forEach((preset, i) => {
            ...
            rect.setAttribute('x', startX + i * (swatchSize + gap));
            ...
            swatch.addEventListener('click', (e) => {
                e.stopPropagation();
                if (isWhite) {
                    applyColor({ ct: preset.ct });
                } else {
                    applyColor({ hue: preset.hue, sat: preset.sat });
                }
            });
     ...
     */
    function createBrightnessSlider(x, y, width, currentBri) {
        const group = document.createElementNS(NS, 'g');
        const height = 20;
        ...
        track.setAttribute('rx', height / 2);
        ...
        trackFill.setAttribute('x', x);
        trackFill.setAttribute('y', y);
        trackFill.setAttribute('width', width);
        trackFill.setAttribute('height', height);
        trackFill.setAttribute('rx', height / 2);
        ...
        valueText.setAttribute('y', y + height / 2 + 4);
     ...
     */
    async function applyColor(state) {
        ...
        try {
            ...
            if (success) {
                ...
                setTimeout(() => {
                    if (window.HomeMonitor && window.HomeMonitor.loadLights) {
                        window.HomeMonitor.loadLights();
                    }
                }, 300);
            } else {
