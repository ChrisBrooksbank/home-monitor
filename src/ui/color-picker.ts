 ...
 */
function createColorWheel(cx: number, cy: number, radius: number, lightData: LightData): SVGGElement {
  const group = document.createElementNS(NS, 'g') as SVGGElement;
 ...
 */
function createSwatchRow(presets: (WhitePreset | ColorPreset)[], _x: number, y: number, isWhite: boolean): SVGGElement {
  const group = document.createElementNS(NS, 'g') as SVGGElement;
  const swatchSize = 24;
  const gap = 8;
 ...
 */
function createBrightnessSlider(x: number, y: number, width: number, currentBri: number): SVGGElement {
  const group = document.createElementNS(NS, 'g') as SVGGElement;
  const height = 20;
