...
(globalThis.window as typeof globalThis & { addEventListener: Mock }).addEventListener =
...
describe('debounce', () => {
  ...
  it('should preserve this context', () => {
    const obj = {
      ...
      debouncedGetValue: null as ReturnType<typeof debounce<() => number>> | null,
    };
...
describe('throttle', () => {
  ...
  it('should preserve this context', () => {
    const obj = {
      ...
      throttledGetValue: null as ReturnType<typeof throttle<() => number>> | null,
    };
  ...
  it('should include timeout signal', async () => {
    ...
    (
      globalThis as typeof globalThis & {
        AbortSignal: { timeout: Mock<[number], string> };
      }
    ).AbortSignal = {
      timeout: vi.fn().mockReturnValue('mock-signal'),
    };

    const globalAppConfig = (globalThis as typeof globalThis & { APP_CONFIG: AppConfig })
      .APP_CONFIG;
    await checkProxyAvailability('http://localhost:3000', 'Test');

    expect(AbortSignal.timeout).toHaveBeenCalledWith(globalAppConfig.timeouts.proxyCheck);
    expect(fetch).toHaveBeenCalledWith(
      'http://localhost:3000',
