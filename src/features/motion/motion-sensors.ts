 ...
 * Log a motion event
 */
export function logMotionEvent(room: string): void {
  const now = Date.now();
  const motionHistory = getMotionHistory();
...
/**
 * Motion sensors module export
 */
export const MotionSensorsModule = {
  getMotionSensors,
  getMotionHistory,
  initMotionHistory,
  logMotionEvent,
...
if (typeof window !== 'undefined') {
  (window as Window & { MotionSensorsModule?: typeof MotionSensorsModule }).MotionSensorsModule =
    MotionSensorsModule;
}
