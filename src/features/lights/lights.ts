...
function getColorPicker() {
    return Registry.getOptional('ColorPicker') as
        | { handleBulbClick: (id: string, light: LightInfoExtended, e: MouseEvent) => void }
        | undefined;
}
...
function updateLightIndicators(): void {
    ...
    for (const [room, lights] of Object.entries(roomLights)) {
        ...
        lights.forEach((light, i) => {
            ...
            const bulbGroup = createPixelBulb(
                ns,
                pos.x + offsetX,
                pos.y,
                light as LightInfoExtended
            );
...
// Register lights functions with the service registry
Registry.register({
    key: 'Lights',
    instance: {
        getRoomLights,
