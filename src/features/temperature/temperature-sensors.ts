 ...
 */
export function getTempHistory(): Record<string, TemperatureHistoryEntry[]> {
 ...
 */
export function saveTempData(room: string, temp: string): void {
  const now = Date.now();
  const tempHistory = getTempHistory();
...
/**
 * Temperature sensors module export
 */
export const TemperatureSensorsModule = {
  getTempHistory,
  initTempHistory,
  saveTempData,
  loadTemperatures,
...
if (typeof window !== 'undefined') {
  (
    window as Window & { TemperatureSensorsModule?: typeof TemperatureSensorsModule }
  ).TemperatureSensorsModule = TemperatureSensorsModule;
}
